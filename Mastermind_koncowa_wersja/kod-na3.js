window.onload = function(){
    ...
    function handleClick(e)
    {
		...
		{
			if(x>xg+40+i*50 && x<xg+40+i*50+30 && y>yp+470-55*pozycja && y<yp+470-55*pozycja+30)
			{
				ctx.fillStyle = kolor;
				ctx.fillRect(xg+40+i*50,yp+470-55*pozycja,30,30);
				ctx.strokeStyle = kolor_ramka;
				ctx.rect(xg+40+i*50,yp+470-55*pozycja,30,30);
				kod_odp[i] = kolor;
			} 
		}	
    ...
    function plansza()
    {
        ...
        ctx.fillRect(xg,yg,400,500);
        ctx.rect(xg,yg,400,500);
                ...
                for(i=0;i<4;i++)
                    {
                        ...
                        ctx.rect(xg+40+i*50,yp+470-55*j,30,30);
                    }
            }
        ...
        for(j=0;j<8;j++)
            {
                for(i=0;i<4;i++)
                    {
                        ctx.rect(xg+255+i*30,yp+478-55*j,15,15);
                    }
            }
        ...
        for(i=1;i<=8;i++)
            {
                ctx.fillText(""+i,xg+22,yg+470-(i-1)*55);
            }	
			
		for(i=0;i<4;i++)
		{
            ctx.fillStyle = "#4D3425";
            ctx.fillRect(xg+40+i*50,yp+470-55*8,30,30);
			ctx.fillStyle = "#696969";
			ctx.fillRect(xg+40+i*50+2,yp+470-55*8+2,26,26);
		}
		...
		for(i=1;i<=4;i++)
        {
            ctx.fillText("?",xg+i*50-4,yp+495-8*55);
        }
	...
	function sprawdzaj(pozycja)
	{
		...
		for(i=0;i<4;i++) //zamalowywanie na bialo lub czarno po odpowiedzi
		{
			...
			{
				ctx.fillRect(xg+255+i*30,yp+478-pozycja*55,15,15);
			}
			
			ctx.strokeStyle = kolor_ramka;
			ctx.rect(xg+255+i*30,yp+478-pozycja*55,15,15);
		}	
	}
		...
		document.getElementById("button1").onclick = function()
		{
			...
			if(licz==4)
			{
				sprawdzaj(pozycja);
                pomocnik++;
                if(pozycja<7 && podpowiedz[0,1,2,3] != 2 ) pozycja++;  //blokowanie moliwosci wstawiania kolorow po wygranej
                tak = true;		
                if(podpowiedz[0,1,2,3] == 2)
                    {
		                ...
		                for(i=0;i<4;i++)
                        {
                            if(i==0) 
                            {
				                ...
				                ctx.fillRect(xg+i*50-4,yp+495-8*55,30,30);
                            }
                            if(i==1) 
                            {
